...
const getFeedName = `-- name: GetFeedName :many
...
func (q *Queries) GetFeedName(ctx context.Context) ([]GetFeedNameRow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFeedNameRow
	for rows.Next() {
